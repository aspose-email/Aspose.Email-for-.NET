...
namespace Aspose.Email.Live.Demos.UI.Helpers
{
    ...
    public static class MapiHelper
    {
		...
		public static Aspose.Email.Mapi.MapiMessage GetMapiMessageFromStream(Stream input)
		{
		...
		public static Aspose.Email.Mapi.MapiMessage GetMapiMessageFromStream(Stream input, out FileFormatInfo info)
		{
			info = Email.Tools.FileFormatUtil.DetectFileFormat(input);
			input.Position = 0;
		...
		public static MailMessage GetMailMessageFromStream(Stream input)
		{
			var formatInfo = Email.Tools.FileFormatUtil.DetectFileFormat(input);
			input.Position = 0;
			...
			{
				case Email.FileFormatType.Msg:
					return MailMessage.Load(input, new Email.MsgLoadOptions());
				case Email.FileFormatType.Eml:
					return MailMessage.Load(input, new Email.EmlLoadOptions());
				case Email.FileFormatType.Mht:
					return MailMessage.Load(input, new Email.MhtmlLoadOptions());
				case Email.FileFormatType.Tnef:
					return MailMessage.Load(input, new Email.TnefLoadOptions());
				default:
					throw new Exception("Invalid file format");
			}
		}
