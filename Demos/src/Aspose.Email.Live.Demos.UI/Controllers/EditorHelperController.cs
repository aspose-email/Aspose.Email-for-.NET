...
namespace Aspose.Email.Live.Demos.UI.Controllers
{
	...
	///</Summary>
	public class EditorHelperController : EmailBase
	{
        ///<Summary>
		...
		public HttpResponseMessage GetHTML(string fileName, string folderName)
        {
			...
			var emailController = new AsposeEmailEditorController();
			var html = emailController.GetHTML(fileName, folderName);
			if (html != null)
		...
		public HttpResponseMessage UpdateContents([FromBody] UpdateContentsRequest request)
		{
			...
			var emailController = new AsposeEmailEditorController();
			var response = emailController.UpdateContents(request.fileName, request.folderName, request.htmldata, request.outputType);
			if (response != null)
