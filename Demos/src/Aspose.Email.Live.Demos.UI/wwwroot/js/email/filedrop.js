	...
	$.fn.filedrop = function (options) {
		...
		var prepareFormData = function (min = 1, max = undefined) {
			...
			if (droppedFiles.length) {
				...
				if (!options.CanWorkWithoutFiles)
					...
					$.ajax({
						...
						error: function (e) {
							...
							if (e.Status === undefined) {
								...
								if (state === 'rejected')
									options.showAlert('Service unavailable, please retry later');
								else
									options.showAlert(state);
							}
							else
