...
namespace Aspose.Email.Live.Demos.UI.Config
{
    ...
    public class GlobalAppHelper
    {
		...
		public GlobalAppHelper(HttpContext hc, HttpApplicationState appState, string SessionID, string language)
		{
			...
			AsposeEmailContext.atcc = new AsposeEmailContext(hc); // sync the context
		...
		private void initResources(string ResourcesFile, string SessionID)
		{
			...
			if (AsposeEmailContext.Current.Cache[SessionID] == null)
			{
				...
				AsposeEmailContext.Current.Cache.Add(
				   SessionID,
					resources,
