...
function requestEditor(data) {
	...
	openIframe(url, '/email/editor', '/email/edit');
}
...
function requestMetadata(data) {
	...
	if (response.length > 0) {
		let url = o.UIBasePath + 'api/AsposeEmailMetadata/properties?folderName=' + response[2] + "&fileName=" + response[1];
		$.ajax({
			type: 'POST',
			url: url,			
...
function requestViewer(data) {
	...
	openIframe(url, '/email/viewer', '/email/view');
}
